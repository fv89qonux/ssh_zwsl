...
<body width="100%"><div id="loader" >页面加载中...</div><div id="result" class="result none"></div><div class="mainbox"><div id="nav" class="mainnav_title"><ul><a href="method!userlist">用户列表</a>|
	...
	<tr><td>用户组</td><td><select name="role" validate="required:true"><option value="0" <c:if test="${user.role == 0}">selected</c:if>>普通用户</option><option value="1" <c:if test="${user.role==1}">selected</c:if>>管理员</option></td></tr>
